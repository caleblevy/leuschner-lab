...
def func_safe(func, args=tuple(), kwargs=dict(), exception=RuntimeError):
    """
    ...
    while obs_status['ready']:
        ...
        try:
            ...
            if alt <= alt_limits[int(round(az))]:
                if debug or verbose:
                    time.asctime()
                    print 'Coordinates below the altitude limit.\n'
                raise ValueError

            if not debug:
                # dish.Dish.point will raise a ValueError if
                func_safe(leuschner.point, (alt, az))
            ...
            if first_attempt:
                ...
                while obs_status['ready']:
                    ...
                    if tr_status['diode'] and not obs_status['want-diode']:
                        func_safe(leuschner.noise_off)
                    elif not tr_status['diode'] and obs_status['want-diode']:
                        func_safe(leuschner.noise_on)
